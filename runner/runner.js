...
async function compileC(sourceCode) {
    ...
    const tempDir = path_1.default.join((0, os_1.tmpdir)(), 'sasi-runner-sandbox');
    await fs_1.promises.mkdir(tempDir, { recursive: true }).catch(() => { });
    const isWindows = process.platform === 'win32';
    const binExt = isWindows ? '.exe' : '.out';
    const srcPath = path_1.default.join(tempDir, `source_${id}.c`);
    const binPath = path_1.default.join(tempDir, `exec_${id}${binExt}`);
    const cleanup = async () => {
        await Promise.all([
            fs_1.promises.unlink(srcPath).catch(() => { }),
            fs_1.promises.unlink(binPath).catch(() => { }),
        ]);
    };
    try {
        await fs_1.promises.writeFile(srcPath, sourceCode, 'utf-8');
    }
    catch (err) {
        await cleanup();
        return {
    ...
    const compileResult = await new Promise((res) => {
        ...
        (0, child_process_1.exec)(compileCmd, { cwd: tempDir, timeout: 5000, maxBuffer: 32 * 1024 }, (err, stdout, stderr) => {
...
function runBinary(binPath, inputData, timeoutMs) {
    ...
    return new Promise((resolve) => {
        ...
        const child = (0, child_process_1.exec)(cmd, {
            ...
            const timeMs = Math.round(perf_hooks_1.performance.now() - startTime);
            if (err) {
...
const socket = (0, socket_io_client_1.io)(`${SERVER_URL}/runner`, {
...
socket.on('runner_benchmark:status', (state) => {
    if (!state)
        return;
    if (state.run) {
...
socket.on('BENCHMARK_EXECUTE_JOB', async (payload) => {
    ...
    const compileStart = perf_hooks_1.performance.now();
    const { binPath, cleanup, securityViolation, compileError } = await compileC(sourceCode);
    const compileDuration = Math.round(perf_hooks_1.performance.now() - compileStart);
...
socket.on('EXECUTE_JOB', async (job) => {
...
const guiServer = http_1.default.createServer((req, res) => {
...
setInterval(() => {
    ...
    const telemetry = getSystemTelemetry();
    process.stdout.write(`\r[Runner Pool] Status: CONNECTED | Run: ${currentRunId} | Workers: ${running}/${CONCURRENCY} Busy | Running: ${running} | Queued: ${queued} | Done: ${totalCompleted} | Fail: ${totalFailed} | RAM: ${telemetry.usedGb}GB | Temp: ${telemetry.pkgTemp}°C `);
}, 500);
